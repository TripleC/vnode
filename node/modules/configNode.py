...
class VNodeNodeConfig:
  ...
  def createVMConfigFile( self, username, machinename, vmNodename, vmHostname, \
                          vmId, vmMemory, vmPartition, vmImageType, configFilename = None):
    try:
      ...
      if vmImageType == "SL-3-32":
        d = dict( machinename = machinename, ip = socket.gethostbyname(vmHostname), \
                  hostname = vmHostname, memory = vmMemory, \
                  vg = self.getVolumeGroupName(), driveroot = 'sda1', \
                  driveswap = 'sda2', vcpus = num,username=username)
      else:
        d = dict( machinename = machinename, ip = socket.gethostbyname(vmHostname), \
                  hostname = vmHostname, memory = vmMemory, vg = self.getVolumeGroupName(),\
                  driveroot = 'xvda1', driveswap = 'xvda2', vcpus = num, username = username)
