...
import configServer, configCommon, pingNode, logger
...
def getReservedHostData():
   ...
   for reservedHostname in reservedNodesHostname:
       ...
       data.append({'PhysicalHost':reservedNodesInfo['physicalhost'], 'VirtualHost':reservedHostname,'ExpiryTime':reservedNodesInfo['expirytime'], 'Memory':reservedNodesInfo['vmmemory'], 'Partition':reservedNodesInfo['vmpartition'], 'OSImage':reservedNodesInfo['vmimagetype'], 'ExpiryTimeAt':expiryReservedTime, 'Name':reservedNodesInfo['vmid']})
   print simplejson.dumps(data_f)

def getPhysicalHostData():
    """Returns a tuple containing the a list of all available physical host,
    the coloring for them and the default physical host."""
...
def reserveVM():
  ...
  #file=open("/tmp/log","w")
  #file.write(str(hostInfo))
  #file.write("\n")
  #file.write(str(physicalHostData[0]))
  #file.close()
  if not hostInfo:
    logging.info(username + "@" +os.getenv('REMOTE_ADDR') +" wants to reserve " + vmHostname + " but hostname does not exist " + vmNodename)
    print "502"
    return
  if vmNodename not in physicalHostData[0]:
    logging.info(username + "@" +os.getenv('REMOTE_ADDR') +" wants to reserve " + vmHostname + " but node does not exist or is not available " + vmNodename)
    print "501"
    return
...
def availableHostnames(query="remote"):
...
def deploy(sitename = "default", repository = "default", vmGliteServices = ['default','none','none','none','none','none']):
  ...
  for i in range(0,size):
    ...
    if vmHostData['state'] == 'reserved':
      ...
      print "Machine cannot be deployed because is not reserved"
  ...
  if stateRequest == 'all':
    ...
    for host in deployed:
      if (host in respondingHosts):
         data.append({'VirtualHost':host,'StateMessage':'deployed','StateColor':'deployed','Message':'Could verify connection to VM'})
      else:
         data.append({'VirtualHost':host,'StateMessage':'deployed','StateColor':'deployedNV','Message':'Could not verify connection to VM'})
    for host in notDeployed:
      data.append({'VirtualHost':host,'StateMessage':'available','StateColor':'available','Message':'None'})
    for host in reserved:
      nodeInfo = state.getVirtualHostInfo(host)
      userReserve = nodeInfo['user']
      data.append({'VirtualHost':host,'StateMessage':'reserved','StateColor':'reserved','Message':'Reserved to '+str(userReserve)})
    for host in failed:
      nodeInfo = state.getVirtualHostInfo(host)
      stateMessage = nodeInfo['statemessage']
      data.append({'VirtualHost':host,'StateMessage':'failed','StateColor':'failed','Message':stateMessage})
    for host in terminating:
      nodeInfo = state.getVirtualHostInfo(host)
      stateMessage = nodeInfo['statemessage']
      data.append({'VirtualHost':host,'StateMessage':'terminating','StateColor':'terminating','Message':stateMessage})
    for host in deploying:
      nodeInfo = state.getVirtualHostInfo(host)
      stateMessage = nodeInfo['statemessage']
      data.append({'VirtualHost':host,'StateMessage':'deploying','StateColor':'deploying','Message':stateMessage})
    #print data
    print simplejson.dumps(data_f)
    ...
    for host in disabled:
      data.append({'VirtualHost':host,'StateMessage':'disabled','StateColor':'disabled','Message':'None'})
    print simplejson.dumps(data_f)
  #print data
...
def main():
  ...
  if form.has_key("vmAction"):
    ...
    if action == "reserve": 
      ...
      print "Server Response: Error - invalid option"
