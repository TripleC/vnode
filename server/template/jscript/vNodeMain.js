...
function disablePortalJS(state) {
  YAHOO.util.Connect.asyncRequest(
    'GET', proto + host + path + '?vmAction=disablePortal&vmOption=' + state,
    null, null);
}

function getUsername() {
 YAHOO.util.Connect.asyncRequest('GET', proto + host + path +'?vmAction=getUsername',
...
function getVGSite(sitename) {
    YAHOO.vnode.Basic10.myDataSource.sendRequest(
        'vmAction=userVG&sitename=' + sitename,
        YAHOO.vnode.Basic10.myDataTable.onDataReturnInitializeTable, 
        YAHOO.vnode.Basic10.myDataTable);
}
...
function getChecked(val,nr) { 
    if ( parseInt( YAHOO.vnode.Basic1.connectionCallback.myDataTable.getRecordSet().getLength() ) + 
         parseInt(nr) > 4) {
        alert("You will have more than 4 machines");
        return;
    }
    YAHOO.vnode.container.wait.show();
    YAHOO.util.Connect.asyncRequest('GET', proto + host + path + '?vmAction=autoHosts&&nrHosts=' + nr,
                                    YAHOO.vnode.Basic1.connectionCallback, null);
}

//function to get the VMs that are in a given state
function getState(state) {
    if (state == 'Deployed'    || state == 'NotDeployed' || state == 'Failed' || 
        state == 'Terminating' || state == 'Reserved'    || state == 'Deploying' || 
        state == 'All'         || state == 'Disabled') {
        YAHOO.util.Connect.asyncRequest('GET', proto + host + path + '?vmAction=state' + state,
                                        YAHOO.vnode.CustomFormatting.connectionCallback2, null)
  } 
};

//Function to verify if portal is blocked
function isPortalBlocked() {
    YAHOO.util.Connect.asyncRequest('GET', proto + host + path + '?vmAction=checkDisablePortal',
                                    callbackPolice, null);
}
...
function getVGState(state) {
    if (state == 'Deployed'    || state == 'Failed' || 
        state == 'Terminating' || state == 'Deploying') {
        YAHOO.vnode.CustomFormatting2.myDataSource.sendRequest(
            'vmAction=stateVG&vgState=' + state, 
            YAHOO.vnode.CustomFormatting2.myDataTable.onDataReturnInitializeTable, 
            YAHOO.vnode.CustomFormatting2.myDataTable);
    }
    if (state == 'All') {
        YAHOO.vnode.CustomFormatting2.myDataSource.sendRequest(
            'vmAction=stateVG',
...
function init() {
  ...
  var AjaxObject = {
    handleSuccess:function(o){
            if (o.responseText == 201) {
                YAHOO.util.Connect.asyncRequest('GET',proto + host + path +'?vmAction=getReservedData',
                                                YAHOO.vnode.Basic1.connectionCallback,null);
            }
            else if (o.responseText == 202) { 
                YAHOO.vnode.Basic2.myDataSource.sendRequest(
                    'vmAction=userVM',
                    YAHOO.vnode.Basic2.myDataTable.onDataReturnInitializeTable, 
                    YAHOO.vnode.Basic2.myDataTable);
            }
        ...
        var elState = new YAHOO.util.Element('allAdmin');
                if (elState.get('value') == 'Select...') {
                    YAHOO.util.Connect.asyncRequest('GET',proto + host + path +'?vmAction=stateAll',
                                                    YAHOO.vnode.CustomFormatting.connectionCallback2,null);
                }
                else
                    YAHOO.util.Connect.asyncRequest(
                        'GET', proto + host + path +'?vmAction=state'+elState.get('value'),
                        YAHOO.vnode.CustomFormatting.connectionCallback2,null);
      }
  ...
  var handleSubmit = function() {
    ...
    if (YAHOO.vnode.Basic1.connectionCallback.myDataTable.getRecordSet().getLength() == 0) {
      alert("Please choose a virtual machine.");
            ...
            for (i = 0; i < YAHOO.vnode.Basic1.connectionCallback.myDataTable.getRecordSet().getLength(); 
                 i++) {  
                ...
                s6 = myArrayName.join();
                s_name = s6.split(' ').join('');
            }
            YAHOO.vnode.container.wait.show();
            YAHOO.util.Connect.asyncRequest('GET', proto + host + path +'?vmAction=deploy&vmOptions=' 
                                            +  s0 + "," + s1 + "," + s2 + "," + s3 + "," + s4 + "," 
                                            + s5 + "," + s_name, callback, null);
        }
  };
      ...
      if(confirm('Are you sure?')) {
        for (i = 0; i<oRecordId.length; i++) {  
          var oRecord = YAHOO.vnode.CustomFormatting.connectionCallback2.myDataTable.getRecordSet().getRecord(oRecordId[i]);
                ...
                YAHOO.util.Connect.asyncRequest('GET', proto + host + path 
                                                +'?vmAction=enableVM&virtualHostnames=' +  s, 
                                                callback, null);
      }
        }
    }
    ...
    if (YAHOO.vnode.Basic1.connectionCallback.myDataTable.getRecordSet().getLength() == 0) {
      alert("Please choose a virtual machine.");
            ...
            YAHOO.vnode.container.wait.show();
            YAHOO.util.Connect.asyncRequest(
                'GET', proto + host + path +'?vmAction=deployVG&vmOptions=' 
                +  s0 + "," + s1 + "," + s2 + "," + s3 + "," + s4 + "," + s5 + "," + s6 
                + '&vmGlite=' + myArrayRow0 + "," + myArrayRow1 + "," + myArrayRow2 + "," 
                + myArrayRow3 + '&vmSitename=' + elS.get('value') + '&vmRepo=' + elR.get('value'), 
                callback, null  );
        }
    };
  ...
  var handleDelete = function() {
    ...
    var handleRefreshStatVgs = function () {
        ...
        if (elState.get('value') == "Select..." || elState.get('value') == "") {
            ...
            YAHOO.vnode.CustomFormatting2.myDataSource.sendRequest(
                'vmAction=stateVG&vgState=' + elState.get('value'),
                YAHOO.vnode.CustomFormatting2.myDataTable.onDataReturnInitializeTable, 
                YAHOO.vnode.CustomFormatting2.myDataTable);
        }
    }
    ...
    var handleTerminateSiteVG = function () {
        ...
        if (elS2.get('value') == "Select..." || elS2.get('value') == "") {
            ...
            YAHOO.vnode.container.wait.show();
            YAHOO.util.Connect.asyncRequest(
                'GET', proto + host + path +'?vmAction=terminateVG&&vmOption=site&vmSite=' 
                + elS2.get('value'), callback, null);
        }
    }
    ...
    if (YAHOO.vnode.Basic1.connectionCallback.myDataTable.getRecordSet().getLength() < 4) {
            var regex_hostnames_1=new RegExp(/vtb-generic-(1[1-9]|2[0-9]|30).cern.ch/)
            var regex_hostnames_2=new RegExp(/vtb-generic-(3[1-9]|[456789][0-9]|10[0-9]|11[0-6]).cern.ch/)
            var regex_hostnames_3=new RegExp(/lxb760([4-8])v([1-5]).cern.ch/)

            if (data.physicalHost != 'Select...' && data.virtualHostnames != 'Select...' && 
                data.expiryTimeVM != 'Select...' && data.memoryVM != 'Select...' && 
                data.partitionVM != 'Select...' && data.osImageVM != 'Select...') {
                
                el = new YAHOO.util.Element('actionScript');
                el.set("value","reserve");
                YAHOO.vnode.container.wait.show();
                var el = new YAHOO.util.Element('vmN');
                el.set('value',randomString()); 
                if (data.imageFilename == "") {
                    YAHOO.util.Connect.asyncRequest(
                        'GET', proto + host + path 
                        + '?vmAction=reserve&physicalHost=' + data.physicalHost
                        + "&virtualHostnames="              + data.virtualHostnames
                        + "&expiryTimeVM="                  + data.expiryTimeVM  
                        + "&memoryVM="                      + data.memoryVM
                        + "&partitionVM="                   + data.partitionVM    
                        + "&vmName="                        + data.vmName
                        + "&osImageVM="                     + data.osImageVM , 
                        callback, null);
                } else {
                    YAHOO.util.Connect.asyncRequest(
                        'GET', proto + host + path 
                        ...
                        + "&partitionVM="      + data.partitionVM 
                        + "&vmName="           + data.vmName 
                        + "&imageFilename="    + data.imageFilename , callback, null);
                }
            } else {
  ...
  YAHOO.vnode.container.dialog11 = new YAHOO.widget.Dialog
        ("dialogDefaultVMs", 
         { width : "30em",
           fixedcenter : false,
           visible : false, 
  ...
  YAHOO.vnode.container.dialog11.callback = { success: handleSuccess, failure: handleFailure };
    
  // Render the Dialog
  YAHOO.vnode.container.dialog11.render();
        
  // Instantiate the deploy VM Dialog
  YAHOO.vnode.container.dialog1 = new YAHOO.widget.Dialog
        ("dialogDeployVMs", 
         { width : "57em",
           fixedcenter : false,
           visible : false, 
  ...
  YAHOO.vnode.container.dialog2 = new YAHOO.widget.Dialog(
        "terminateVMs", 
        { width : "65em",
          fixedcenter : false,
          visible : false, 
          ...
          close:true,
          underlay: "none",
          buttons : [ { text:"Refresh", handler:handleRefreshTerm, isDefault:true },
                      { text:"Terminate", handler:handleTerminate }]
        });
  ...
  YAHOO.vnode.container.dialog3 = new YAHOO.widget.Dialog(
        "stateOfVMs", 
        { width : "50em",
          fixedcenter : false,
          visible : false, 
  ...
  YAHOO.vnode.container.dialog4 = new YAHOO.widget.Dialog(
        "upTimeVMs", 
        { width : "55em",
          fixedcenter : false,
          visible : false, 
  ...
  YAHOO.vnode.container.dialog7 = new YAHOO.widget.Dialog(
        "terminateVGs", 
        { width : "35em",
          fixedcenter : false,
          visible : false, 
          ...
          close:true,
          underlay: "none",
          buttons : [ { text:"Terminate", handler:handleTerminateVG },
                      { text:"Terminate Site", handler:handleTerminateSiteVG }, 
                      { text:"Terminate All", handler:handleTerminateAllVG}, 
                      { text:"Refresh", handler:handleRefreshVG, isDefault:true}]
        });
  ...
  YAHOO.vnode.container.dialog8 = new YAHOO.widget.Dialog(
        "admins", 
        { width : "50em",
          fixedcenter : false,
          visible : false, 
          ...
          close:true,
          underlay: "none",
          buttons : [ { text: "Terminate", handler:handleTerminateAdmin}, 
                      { text: "Refresh", handler:handleRefreshAdmin}, 
                      { text: "Disable", handler:handleDisable},
                      { text: "Enable", handler:handleEnable}]
        });
  ...
  YAHOO.vnode.container.dialog_service_type = new YAHOO.widget.Dialog("serviceType", 
    ...
    var h_kl_service_type = new YAHOO.util.KeyListener(
        document, { alt:true, shift: true,keys:74 },
  ...
  // Instantiate the deploy HC Dialog
  YAHOO.vnode.container.dialog10 = new YAHOO.widget.Dialog
        ("startHCs", 
         { width : "45em",
           fixedcenter : false,
           visible : false, 
           ...
           close:true,
           //underlay: "none",
           buttons: [ { text:"Deploy Hadoop Cloud", handler:handleSubmitVG, isDefault:true } ]
         });
    
  // Render the Dialog
  YAHOO.vnode.container.dialog10.render();
    ...
    var dd10 = new YAHOO.util.DDProxy("startHCs_h");
    
    (dd10.endDrag = function() {
        getXY = YAHOO.util.Dom.getXY(dd10.getDragEl());
        YAHOO.util.Dom.setXY("startHCs_c",getXY,false);
    }); 
  ...
  //shift-J
    var kl_service_type = new YAHOO.util.KeyListener(document, { alt:true, shift:true, keys:s }, 
                                                     { fn:fnCallback,  
                                                       scope:YAHOO.vnode.container.dialog_service_type, 
                                                       correctScope:true } );  
    kl_service_type.enable();
     ...
     //shift-M
    var kl10 = new YAHOO.util.KeyListener(document, { alt:true, shift:true, keys:77 }, 
                                         { fn:fnCallback,  
                                           scope:YAHOO.vnode.container.dialog10, 
                                           correctScope:true } );  
    kl10.enable();
  ...
  function fnCallback(e) {
    ...
    else if  (this.id == "startHCs"){
         if (e == "keyPressed") {
                YAHOO.vnode.container.dialog10.show();
            }
      YAHOO.vnode.container.manager.focus(YAHOO.vnode.container.dialog10);
      } 
    }
    //ok
  ...
  YAHOO.util.Event.addListener("startHCs", "click", fnCallback);
    
    //ok
    (dd1.startDrag = function() {
        YAHOO.util.Dom.setStyle( dd1.getDragEl(), 'height',
                                 YAHOO.util.Dom.getStyle("dialogDeployVMs", 'height') );
    });
    
    (dd2.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd2.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("terminateVMs", 'height') );
    });
    
    (dd3.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd3.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("stateOfVMs", 'height') );
    });
    
    (dd4.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd4.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("upTimeVMs", 'height') );
    });
    
    (dd5.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd5.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("startVGs", 'height') );
    });
    
    (dd6.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd6.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("stateVGs", 'height') );
    });
    
    (dd7.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd7.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("terminateVGs", 'height') );
    });
    
    (dd8.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd8.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("admins", 'height') );
    });
    
    (dd9.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd9.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("siteInfo", 'height') );
    });
  
  (dd10.startDrag = function() {
        YAHOO.util.Dom.setStyle(dd10.getDragEl(), 'height',
                                YAHOO.util.Dom.getStyle("startHCs", 'height') );
    }); 
    //ok
    ...
    YAHOO.vnode.container.manager.register( [YAHOO.vnode.container.dialog1,
          				     ...
          				     YAHOO.vnode.container.dialog10,
                                             YAHOO.vnode.container.dialog11,
                       ]);
